...
final class Store: ObservableObject {
    @Published private(set) var items: [AlarmEntry] = []
    ...
    private let fileName = "earlywake_items.json"
    ...
    func add(_ item: AlarmEntry) -> Bool {
    ...
    private func load() {
        ...
        if let decoded = try? JSONDecoder().decode([AlarmEntry].self, from: data) {
            items = decoded
        }
    }
